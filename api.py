...
class Email:
    ...
    @staticmethod
    def fetching_message(email: str, message_id: str):
        """
        :param email: Email address
        ...
        """
        email = get_login(email)
        request = requests.get(f'https://www.1secmail.com/api/v1/?action=readMessage&login={email[0]}&domain={email[1]}&id={message_id}').json()
        return request

    @staticmethod
    def attachment_download(email: str, message_id: str, file: str):
        """
        :param email: Email address
        :param message_id: message_id. Takes from func check_mailbox
        ...
        """
        email = get_login(email)
        request = requests.get(f'https://www.1secmail.com/api/v1/?action=download&login={email[0]}&domain={email[1]}&id={message_id}&file={file}')
        return request
    ...
    def message_handler(email: str):
        ...
        :return: Dict:
            ...
            htmlBody	Message body (html)
        """
        while not Email.check_mailbox(email):
            sleep(1)
        return Email.fetching_message(email, Email.check_mailbox(email)[0]['id'])
